...
void update_ipv6_address(const char *ifname, const char *ip, const char *gw, unsigned prefix) {
    ...
    if (!access("/sbin/ip", X_OK)) {
        ...
        //ping6 www.qq.com
        snprintf(shell_cmd, sizeof(shell_cmd), "ip -%d route add default dev %s metric 70", 6, ifname);
        ql_system(shell_cmd);
        FILE *file;
        file = fopen("/run/quectel_dev", "w");
        if(file==NULL) {
...
static void update_ip_address_by_qmi(const char *ifname, const IPV4_T *ipv4, const IPV6_T *ipv6) {
    ...
    if(getenv("IPUP_SCRIPT")) {
        char shell_cmd[1024];
        snprintf(shell_cmd, sizeof(shell_cmd), "%s %s", getenv("IPUP_SCRIPT"), ifname);
        ql_system(shell_cmd);
    }
}
