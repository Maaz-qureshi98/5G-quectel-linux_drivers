...
void update_resolv_conf(int iptype, const char *ifname, const char *dns1, const char *dns2) {
    printf("update_resolv_conf: iptype=%i, ifname=%s, dns1=%s, dns2=%s\n", iptype, ifname, dns1, dns2);
    char buffer[256];
    snprintf(buffer, 256, "resolvectl dns %s %s %s",ifname,dns1,dns2);
    printf("%s\n",buffer);
    int res = system(buffer);
    printf("resolvectl returned %i\n",res);
    return;
